...
if [[ -f "$file_a" ]] && [[ -f "$file_b" ]]; then
  ...
  calculate_changes "$file_a" "$mes" "$pattern" #self-defined function
...
if [[ "$recursive" = true ]] && [[ -d "$1" ]] && [[ -d "$2" ]]; then
  ...
  if [[ "$compare_hidden" = true ]] && [[ "$treat_symlinks" = true ]]; then
      echo "here -a -l"
      diff_info=`diff -r "$1" "$2"`
  elif [[ "$treat_symlinks" = true ]]; then
      echo "here -l"
      diff_info=`diff -r --exclude='.*' "$1" "$2"`
  elif  [[ "$compare_hidden" = true ]]; then
      echo "here -a"
      diff_info=`diff -r --exclude='$(type -l)' "$1" "$2"`
  else
      echo "here"
      diff_info=`diff -r --exclude='.*' --exclude='$(type -l)'`
  fi
  ...
  echo "$diff_info" | while IFS= read -r line; do
    ...
    if [[ $line == diff* ]]; then
        ...
        file=$(echo "$line" | awk '{print $((NF - 1))}' | xargs basename)
...
elif [ "$recursive" = false ] && ([ "$compare_hidden" = true ] || [ "${#regex_exp}" -gt 0 ]); then
  echo "Error: -a and -n options require -r option."
  usage #call usage function
